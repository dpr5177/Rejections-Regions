  ...
  doPlot <- function(){
    ...
    if (input$dist=="t") {
      ...
      plottitle <- paste("Rejection region based on ",dist(),
                         " distribution with df=",input$tdf," and alpha=",formatC(input$alpha,3),sep="")
      ...
      plottitle <- paste("Rejection region based on ",dist(),
                         " distribution and alpha=",formatC(input$alpha,3),sep="")
    } else {
    ...
    if (tails==-1 || tails==2) {
      ...
      rejplot <- rejplot + geom_ribbon(data=subset(dd,x <= critvalue),aes(ymax=pdf),ymin=0,
        ...
        geom_vline(aes(xintercept=critvalue), color="blue", linetype="dashed", size=1)
